...
session_manager_fi = MCPSessionManager()
session_manager_booking = MCPSessionManager
...
async def lifespan(app: FastAPI):
    ...
    yield
    # Shutdown
    await session_manager_fi.disconnect()
    await session_manager_booking.disconnect()
...
async def root():
    """Root endpoint"""
    fi_server_url = os.getenv("FI_MCP_SERVER")
    booking_agent_server_url = os.getenv("BOOKING_MCP_SERVER")
    
    try:
        await session_manager_booking.connect(booking_agent_server_url)
        await session_manager_fi.connect(fi_server_url)
        
        return {
            "success": True,
            "message": f"Connected to {fi_server_url} {booking_agent_server_url}",
            "server_url": [fi_server_url, booking_agent_server_url]
        }
    except Exception as e:
...
async def get_status():
    ...
    return {
        "connected": [session_manager_fi.is_connected, session_manager_booking.is_connected],
        "server_url": [session_manager_fi.server_url, session_manager_booking.server_url]
    }
...
async def list_tools():
    ...
    return await session_manager_fi.list_tools().append(session_manager_booking.list_tools())

@app.post("/tools/call", response_model=ToolCallResponse)
async def call_tool(request: ToolCallRequest):
    """Call a tool on the MCP server"""
    try:
        if request.tool_name in session_manager_fi.list_tools():
            result = await session_manager_fi.call_tool(request.tool_name, request.arguments)
        if request.tool_name in session_manager_booking.list_tools():
            result = await session_manager_fi.call_tool(request.tool_name, request.arguments)
...
async def process_query(request: QueryRequest):
    ...
    try:
        result = await session_manager_fi.process_query(request.query)
        return {
            "success": True,
            "response": result
        }
...
async def get_tool_info(tool_name: str):
    ...
    tools = await session_manager_fi.list_tools()
    tools += await session_manager_booking.list_tools()
