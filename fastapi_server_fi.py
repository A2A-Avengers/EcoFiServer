...
def clean_schema(schema):
    """removes the field Title and $schema from config; Since they are forbidden in Gemini"""
...
async def root():
    """Root endpoint"""
    fi_server_url = os.getenv("BOOKING_MCP_SERVER")
    try:
        await session_manager.connect(fi_server_url)
        return {
            "success": True,
            "message": f"Connected to {fi_server_url}",
            "server_url": fi_server_url
        }
    except Exception as e:
